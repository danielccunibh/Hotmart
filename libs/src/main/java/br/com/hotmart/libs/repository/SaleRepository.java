...
public interface SaleRepository extends PagingAndSortingRepository<Sale, Long> {
	...
	@Query("SELECT new br.com.hotmart.libs.dto.ProductValueDTO(p.id, (SUM(s.evaluation) / COUNT(id))) FROM Sale s INNER JOIN s.product p WHERE s.dateSale BETWEEN :dateInitial AND :dateFinal AND p.id IN (:ids) GROUP BY p.id ")
